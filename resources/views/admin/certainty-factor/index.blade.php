...
@section('content')
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Daftar Certainty Factor</h1>
            <a href="{{ route('admin.certainty-factor.create') }}"
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            <div class="overflow-x-auto">
                <table id="certaintyFactorTable" class="min-w-full divide-y divide-gray-400">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">No</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kode Gejala</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Gejala</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">CF Pakar</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($certaintyFactors as $index => $cf)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                    {{ $cf->gejala->nama_gejala }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ number_format($cf->cf_pakar, 1) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('admin.certainty-factor.edit', $cf->id) }}"
                                        ...
                                        <form action="{{ route('admin.certainty-factor.destroy', $cf->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                onclick="return confirm('Apakah Anda yakin ingin menghapus certainty factor ini?')"
