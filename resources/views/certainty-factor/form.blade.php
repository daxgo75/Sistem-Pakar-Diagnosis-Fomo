<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <!--Header Section -->
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 -m-6 p-6 mb-6 border-b">
                    <div class="max-w-3xl mx-auto text-center">
                        <h2 class="text-3xl font-bold text-gray-900 mb-2">Diagnosis FOMO</h2>
                        <p class="text-gray-600">Jawab pertanyaan di bawah ini sesuai dengan kondisi yang Anda alami
                        </p>
                        <div
                            class="mt-4 inline-flex items-center px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Pilih tingkat keyakinan untuk setiap gejala
                        </div>
                    </div>
                </div>

                <form action="{{ route('certainty-factor.calculate') }}" method="POST">
                    @csrf
                    <div class="space-y-4">
                        @foreach ($gejalas as $gejala)
                            <div class="bg-gray-50 p-4 rounded-lg">
                                <div class="flex items-center justify-between gap-4">
                                    <div class="flex-1">
                                        <h3 class="font-medium text-gray-900">{{ $gejala->nama_gejala }}</h3>
                                    </div>
                                    <div class="flex-shrink-0 w-64">
                                        <input type="hidden" name="gejala[{{ $gejala->id }}][gejala_id]"
                                            value="{{ $gejala->id }}">
                                        <select name="gejala[{{ $gejala->id }}][cf_user]"
                                            class="w-full rounded-md border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                                            <option value="">Pilih Kondisi</option>
                                            <option value="1.0">Sangat Yakin</option>
                                            <option value="0.8">Yakin</option>
                                            <option value="0.6">Cukup Yakin</option>
                                            <option value="0.4">Sedikit Yakin</option>
                                            <option value="0.2">Kurang Yakin</option>
                                            <option value="0">Tidak Yakin</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        @endforeach

                        <div class="flex justify-end mt-6">
                            <button type="submit"
                                class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">
                                Proses Diagnosis
                            </button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
</x-app-layout>
